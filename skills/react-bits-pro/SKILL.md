...
| Registry | Contains | Install prefix |
|---|---|---|
| `@reactbits-starter` | 101 animated components (each in 2 variants) | `@reactbits-starter/<slug>-tw` or `-css` |
| `@reactbits-pro` | 238 page-section blocks (21 categories) | `@reactbits-pro/<slug>` |

This installation has the **Ultimate** tier, so every component, block, and template is available.
...
| Type | Source | Suffix | Delivery |
|---|---|---|---|
| **Component** | `@reactbits-starter` | `-tw` / `-css` (required) | shadcn CLI |
| **Block** | `@reactbits-pro` | none | shadcn CLI |
| **Template** | website download | n/a | `.zip` download (login required) |
...
```bash
REACTBITS_LICENSE_KEY=rbpu-xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx
```
...
- If the CLI cannot find the variable, export it in the shell before running `add`:
  ```bash
  export REACTBITS_LICENSE_KEY=rbpu-...
  ```
- This license is the **Ultimate** tier (prefix `rbpu-`), which unlocks everything.
...
1. Be **logged in** at https://pro.reactbits.dev.
2. Download the `.zip` from the template's page.
3. Unzip it and follow its own README to install and run.

| Template | Slug |
|---|---|
| SaaS Landing | `saas-landing` |
| AI SaaS Landing | `ai-saas-landing` |
| Minimal Landing | `minimal-landing` |
| Finance Landing | `finance-landing` |
| Agency Site | `agency-site` |
| Shader Template | `shader-template` |
| Wireframe Template | `wireframe-template` |
| 8 Bit Template | `8-bit-template` |
| AI App Template | `ai-app-template` |
| Security Template | `security-template` |
| Portfolio Template | `portfolio-template` |

The Ultimate license (as configured here) unlocks all of these templates.
...
| `403 Forbidden — Insufficient tier` | Key is not for the Ultimate tier, or access to the item isn't covered | Verify the key is the active Ultimate license; it should cover all components, blocks, and templates. |
...
## Appendix A — Component catalog (`@reactbits-starter`, 101)
...
## Appendix B — Block catalog (`@reactbits-pro`, 238)
...
- **License:** Ultimate — every component, block, and template is available
