# Animation Vocabulary Reference
...
2. **Lead with the glossary term and definition.** Keep wording close for consistency, but add context when browser, library, or implementation details make a definition conditional.
...
- **Spring** — Motion modeled with physical or spring-like parameters; APIs may expose stiffness/damping or a perceptual duration/bounce.
...
- **Velocity** — How fast and in which direction an element is moving. Spring implementations can preserve it when retargeted, allowing a flicked element to carry speed into the next state.
...
- **Compositing** — Combining rendered layers into the final frame; eligible animations may avoid repeated layout or paint, depending on browser and property.
- **will-change** — A CSS hint about an upcoming property change that may help the browser prepare resources; it does not guarantee layer promotion.
- **Layout thrashing** — Repeated DOM reads and writes that force synchronous layout recalculation before the browser can batch the work.
...
- **Hardware acceleration** — Eligible work may be composited or processed by the GPU; `transform` and `opacity` are common candidates, not guarantees.
